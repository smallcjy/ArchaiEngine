...
func InitLog() {
	...
	infoWriter := getWriter(fmt.Sprintf("%s/%s.log", conf.ServerConfig.LogPath, conf.ServerConfig.AppName), conf.ServerConfig.LogTime)
	warnWriter := getWriter(fmt.Sprintf("%s/%s_error.log", conf.ServerConfig.LogPath, conf.ServerConfig.AppName), conf.ServerConfig.LogTime)

	var cores zapcore.Core
	// 最后创建具体的Logger
	if conf.ServerConfig.IsDebug {
