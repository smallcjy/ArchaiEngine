...
func NewTimeWheelOld(interval time.Duration, slotNums, workerNum int) (*TimeWheel, error) {
...
func (tw *TimeWheel) getPosAndCircleByCreatedTime(createdTime time.Time, delay time.Duration) (int, int) {
	...
	passedSeconds := int(passedTime)
	d := int(delay)
	interval := int(tw.interval)

	circle := d / interval / tw.slotNums
	pos := (tw.currentPos + (d-(passedSeconds%interval))/interval) % tw.slotNums
